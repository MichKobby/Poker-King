...
export default function Leaderboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="poker-hero-image h-48 flex items-center justify-center relative">
        <div className="text-center z-10 relative">
          <div className="flex items-center justify-center mb-4">
            <Trophy className="h-12 w-12 text-yellow-400 mr-4" />
            <h1 className="text-4xl font-bold text-white">Poker Night Master</h1>
            <Trophy className="h-12 w-12 text-yellow-400 ml-4" />
          </div>
          <p className="text-yellow-200 text-lg">Friday Night Legends Leaderboard</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Wall of Shame */}
        <div className="poker-card p-6 hover-lift">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-red-400">Wall of Shame</h3>
            <TrendingDown className="h-6 w-6 text-red-400" />
          </div>
          {wallOfShame ? (
            <div className="text-center">
              <p className="text-2xl font-bold text-red-300">{wallOfShame.name}</p>
              <p className="text-red-400">{formatCurrency(wallOfShame.net_profit)}</p>
              <p className="text-sm text-gray-400">{wallOfShame.games_played} games</p>
            </div>
          ) : (
            <p className="text-gray-400 text-center">No losses yet!</p>
          )}
        </div>
        ...
        <div className="poker-card p-6 hover-lift">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-poker-gold-400">Shark of the Month</h3>
            <TrendingUp className="h-6 w-6 text-poker-gold-400" />
          </div>
          {sharkOfTheMonth ? (
            <div className="text-center">
              <p className="text-2xl font-bold text-poker-gold-300">{sharkOfTheMonth.name}</p>
              <p className="text-poker-gold-400">{formatCurrency(sharkOfTheMonth.recent_profit)}</p>
              <p className="text-sm text-gray-400">Last 30 days</p>
            </div>
          ) : (
            <p className="text-gray-400 text-center">No recent games</p>
          )}
        </div>
        ...
        <div className="poker-card p-6 hover-lift">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-poker-green-400">Total Players</h3>
            <Users className="h-6 w-6 text-poker-green-400" />
          </div>
          <div className="text-center">
            <p className="text-3xl font-bold text-poker-green-300">{leaderboard.length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="poker-card-premium hover-lift">
        <div className="p-6 border-b border-poker-gold-400/20">
          ...
          <p className="text-poker-felt-400 mt-2">Your poker group's hall of fame and shame</p>
        </div>
        ...
        {leaderboard.length === 0 && (
          <div className="p-8 text-center">
            ...
            <p className="text-gray-500">Start by adding your first poker night!</p>
          </div>
        )}
      </div>
    </div>
